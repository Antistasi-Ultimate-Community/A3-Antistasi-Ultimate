	...
	_groupX = _x;
	{
		if (alive _x) then
		{
			...
			if (_backpck != "") then
			{
				...
				switch (true) do
				{
					case (_assemblesTo in (FactionGetTieredFT(A3A_faction_reb, "staticMortars", 0))): {_resourcesFIA = _resourcesFIA + ([_assemblesTo] call A3A_fnc_vehiclePrice)/2};
					case (_assemblesTo in (FactionGetTieredFT(A3A_faction_reb, "staticAA", 0))): {_resourcesFIA = _resourcesFIA + ([_assemblesTo] call A3A_fnc_vehiclePrice)/2};
					case (_assemblesTo in (FactionGetTieredFT(A3A_faction_reb, "staticMGs", 0))): {_resourcesFIA = _resourcesFIA + ([_assemblesTo] call A3A_fnc_vehiclePrice)/2};
					case (_assemblesTo in (FactionGetTieredFT(A3A_faction_reb, "staticAT", 0))): {_resourcesFIA = _resourcesFIA + ([_assemblesTo] call A3A_fnc_vehiclePrice)/2};
				};
			};
		};
		deleteVehicle _x;

/*
	...
	Adapted to use Antistasi custom hints
*/
...
// Helper to check if signal is UAV
sa_is_signal_uav = {
	params ["_signal"];
	private _result = 0;
...
// Function to add unit to jamming list (local to client)
fnc_sa_local_add_to_jamm_list = {
	params ["_unit"];
	if (side _unit == side player) then {
		sa_local_jamm_buffer pushBackUnique _unit;
	};
};
...
// Main loop for scan completion and jamming management
_g = [] spawn {

...
if (getMarkerPos _x distance _positionX < distanceSPWN) then
	...
	if (count _staticsX > 0) then
		{
		_threat = _threat + ({typeOf _x in FactionGetTieredFT(A3A_faction_reb, "staticMortars", 0)} count _staticsX) + (2*({typeOf _x in FactionGetTieredFT(A3A_faction_reb,"staticAT", 0)} count _staticsX))
		};
	};

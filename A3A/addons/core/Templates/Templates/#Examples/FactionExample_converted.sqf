...
_vehiclesData set ["vehiclesSDV", ["B_SDV_01_F"]]; //used only in salvage mission and only if template has "vanilla" flag 
/// can be "B_SDV_01_F", "O_SDV_01_F" or "I_SDV_01_F"

//_vehiclesData set ["vehiclesDropPod", ["SpaceshipCapsule_01_F"]]; //used in QRF orbital and crashsite mission
...
_vehiclesData set ["vehiclesBasic", []];			 // unarmed or armed, with 0-2 passengers
_vehiclesData set ["vehiclesLightUnarmed", []];		 // must be unarmed, unarmoured to lightly armoured, with 0-4 passengers
_vehiclesData set ["vehiclesLightArmed", []];             // Should be armed, unarmoured to lightly armoured, with 0-4 passengers
_vehiclesData set ["vehiclesTrucks", []];		 // vehicle that can carry troops and cargoboxes
_vehiclesData set ["vehiclesCargoTrucks", []];		 // vehicle that can carry only cargoboxes
_vehiclesData set ["vehiclesAmmoTrucks", []];		 // wheeled vehicle with capability to rearm vehicles
_vehiclesData set ["vehiclesRepairTrucks", []];		 // wheeled vehicle with capability to repair vehicles
_vehiclesData set ["vehiclesFuelTrucks", []];		 // wheeled vehicle with capability to refuel vehicles
_vehiclesData set ["vehiclesMedical", []];		 // vehicle with capability to provide healing
_vehiclesData set ["vehiclesLightAPCs", []];             // armed, lightly armoured, with 6-8 passengers 
_vehiclesData set ["vehiclesAPCs", []];                  // armed with enclosed turret, armoured, with 6-8 passengers
_vehiclesData set ["vehiclesAirborne", []];              // airborne vehicles, could be with passenger seats or just a crew 
_vehiclesData set ["vehiclesIFVs", []];                  // capable of surviving multiple rockets, cannon armed, with 6-8 passengers
_vehiclesData set ["vehiclesTanks", []];                 // cannon armed, heavely armored, passengers will be ignored
_vehiclesData set ["vehiclesLightTanks", []];             // tanks with poor armor and weapons
_vehiclesData set ["vehiclesAA", []];                    // ideally heavily armed with anti-ground capability and enclosed turret. Passengers will be ignored

_vehiclesData set ["vehiclesTransportBoats", []];	// boat that can carry passengers and cargoboxes
_vehiclesData set ["vehiclesGunBoats", []];              // armed boat, with passengers(?)
//_vehiclesData set ["vehiclesAmphibious", []];          // armed or unarmed wheled or tracked based vehicle with light armor(?) and passengers(?)

_vehiclesData set ["vehiclesPlanesCAS", []];             // Will be used with CAS script, must be defined in setPlaneLoadout. Needs fixed gun and either rockets or missiles
_vehiclesData set ["vehiclesPlanesAA", []];              //Will be used with ASF script, must be defined in setPlaneLoadout.
//Needs fixed gun and either rockets or missiles
_vehiclesData set ["vehiclesPlanesTransport", []];	//Plane that can carry passengers and cargo(?), infantry variant if availbe 
//no need for vehicle variant currently
_vehiclesData set ["vehiclesPlanesGunship", []];     // planes like V-44X armed, AC-130 or pelican from OPTRE, used in GUNSHIP support
//probably can also be a helicopter

_vehiclesData set ["vehiclesHelisLight", []];            // ideally fragile & unarmed helis seating 4+
_vehiclesData set ["vehiclesHelisTransport", []];        // bigger heli with more passengers. 
//Should be capable of dealing damage to ground targets without additional scripting

// Should be capable of dealing damage to ground targets without additional scripting
_vehiclesData set ["vehiclesHelisLightAttack", []];      // Utility helis with fixed or door guns + rocket pods
_vehiclesData set ["vehiclesHelisAttack", []];           // Proper attack helis: Apache, Hind etc

_vehiclesData set ["vehiclesAirPatrol", []];             // preferably light helicopters(armed or unarmed), used in base patrol near bases
...
_vehiclesData set ["uavsAttack", []];                    // unmanned aerial vehicle with heavy armament
_vehiclesData set ["uavsPortable", []];                  // unmanned aerial vehicle(drone), unarmed or armed(Western Sahara style), must be able to be disassembled


//Config special vehicles
              // Militia APCs will be used at roadblocks and attacks at first 4 war levels

_vehiclesData set ["vehiclesPolice", []];                // cars used by police forces

_vehiclesData set ["staticMGs", []];                     // static machine guns
_vehiclesData set ["staticAT", []];                      // static anti-tank weapons 
_vehiclesData set ["staticAA", []];                      // static anti-aircraft weapons
_vehiclesData set ["staticMortars", []];                 // static mortars
_vehiclesData set ["staticHowitzers", []];               // static howitzers

_vehiclesData set ["vehicleRadar", ""];                  // vehicle with radar
_vehiclesData set ["vehicleSam", ""];                    // vehicle with SAM

_vehiclesData set ["howitzerMagazineHE", ""];            // explosive ammo for Howitzer

_vehiclesData set ["mortarMagazineHE", ""];              // explosive ammo for mortars
_vehiclesData set ["mortarMagazineSmoke", ""];           // smoke ammo for mortars

//Minefield definition
//CFGVehicles variant of Mines are needed "ATMine", "APERSTripMine", "APERSMine"
_vehiclesData set ["minefieldAT", []];                   // anti-tank mines
_vehiclesData set ["minefieldAPERS", []];
...
_militiaVehiclesData set ["vehiclesTrucks", []];
_militiaVehiclesData set ["vehiclesLightUnarmed", []];
_militiaVehiclesData set ["vehiclesAPCs", []];

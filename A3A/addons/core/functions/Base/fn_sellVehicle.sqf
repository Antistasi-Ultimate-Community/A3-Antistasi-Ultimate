...
private _costs = call {
    ...
    if (_typeX in flatten (FactionGet(all,"vehiclesReb") + FactionGet(reb,"vehiclesCivCar") + FactionGet(reb,"vehiclesCivTruck"))) exitWith { ([_typeX] call A3A_fnc_vehiclePrice) / 2 };

    private _rebAa = flatten FactionGet(reb, "vehiclesAA");
    ...
    if (
        ...
        or (_typeX in flatten (FactionGet(reb,"vehiclesCivBoat") + FactionGet(reb,"vehiclesCivCar") + FactionGet(reb,"vehiclesCivTruck")))
    ) exitWith {100};
    if (
        (_typeX in flatten FactionGet(all,"vehiclesLight"))
        or (_typeX in flatten OccAndInv("vehiclesTrucks"))
        or (_typeX in flatten OccAndInv("vehiclesCargoTrucks"))
        or (_typeX in flatten FactionGet(reb,"vehiclesTruck"))
    ) exitWith {750};
    if (
        (_typeX in flatten FactionGet(all,"vehiclesBoats"))
        or (_typeX in flatten FactionGet(all,"vehiclesLightAPCs"))
        or (_typeX in flatten OccAndInv("vehiclesAmmoTrucks"))
        or (_typeX in flatten OccAndInv("vehiclesRepairTrucks"))
        or (_typeX in flatten OccAndInv("vehiclesFuelTrucks"))
        or (_typeX in flatten OccAndInv("vehiclesMedical"))
    ) exitWith {1500};
    if (_typeX in flatten [FactionGet(reb,"vehiclesCivHeli")]) exitWith {([_typeX] call A3A_fnc_vehiclePrice) / 2};
    if (_typeX in flatten FactionGet(all,"vehiclesHelisLight")) exitWith {3000};
    if (
        (_typeX in flatten FactionGet(all,"vehiclesAPCs"))
        || (_typeX in flatten FactionGet(all,"vehiclesIFVs"))
        || (_typeX in flatten FactionGet(all,"vehiclesHelisLightAttack"))
        || (_typeX in flatten FactionGet(all,"vehiclesTransportAir"))
        || (_typeX in flatten FactionGet(all,"vehiclesUAVs"))
    ) exitWith {2500};
    if (_typeX in flatten FactionGet(all,"vehiclesLightTanks")) exitWith {3500};
    if (
        (_typeX in flatten FactionGet(all,"vehiclesHelisAttack"))
        or (_typeX in flatten FactionGet(all,"vehiclesTanks"))
        or (_typeX in flatten FactionGet(all,"vehiclesAA"))
        or (_typeX in flatten FactionGet(all,"vehiclesArtillery"))
    ) exitWith {6500};
    if (_typeX in flatten (FactionGet(all,"vehiclesPlanesCAS") + FactionGet(all,"vehiclesPlanesAA") + FactionGet(all,"vehiclesPlanesLargeAA") + FactionGet(all,"vehiclesPlanesLargeCAS"))) exitWith {7500};
    if (_typeX in flatten (FactionGet(all,"vehiclesPlanesGunship"))) exitWith {10000};
    0;
};
